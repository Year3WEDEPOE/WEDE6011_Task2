...
$AdminExists = $conn("select 1 from tbladmin");
$UserExists = $conn("select 1 from tblusers");
$BooksExists = $conn("select 1 from tblbooks");
$OrderExists = $conn("select 1 from tblaorder");

if($AdminExists || $UserExists|| $BooksExists|| $OrderExists !== FALSE)
{
   //Drop Tables
$delete= $conn("DROP TABLE tbladmin");
$delete= $conn("DROP TABLE tbluser");
$delete= $conn("DROP TABLE tblbooks");
$delete= $conn("DROP TABLE tblaorder");
}
else
{
   // sql to create table
...
$qry = "insert into tblAdmin (AdminId, AdminName, APassword, AConfirmPassword, AEmail) values('".$AdminId."';'".$AdminName."';'".$APassword."';'".$AConfirmPassword."';'".$AEmail."')";
...
$qry = "insert into tblUser (StudNum, StudName, SPassword, SConfirmPassword, SEmail) values('".$StudNum."';'".$StudName."';'".$SPassword."';'".$SConfirmPassword."';'".$SEmail."')";
mysqli_query($conn,$qry);
}
